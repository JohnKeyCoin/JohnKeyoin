...
uint256 hashGenesisBlock("0xbabe390e34bdd8e285cc6b582fa62bcf5503fea9ebbe5036a5bcf923cb300424");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // JohnKeyoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "JohnKeyoin Signed Message:\n";
...
{
    // JohnKeyoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // JohnKeyoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // JohnKeyoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // JohnKeyoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbabe390e34bdd8e285cc6b582fa62bcf5503fea9ebbe5036a5bcf923cb300424");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1400714278;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086683450;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // JohnKeyoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JohnKeyoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JohnKeyoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JohnKeyoinMiner(CWallet *pwallet)
{
    printf("JohnKeyoinMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("johnkeyoin-miner");
    ...
    try { loop {
        ...
        printf("Running JohnKeyoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JohnKeyoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JohnKeyoinMiner, pwallet));
}
